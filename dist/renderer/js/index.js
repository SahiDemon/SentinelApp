...
var supabaseUrl = "https://qackdhpbvfbeyhxovlqj.supabase.co";
var supabaseAnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFhY2tkaHBidmZiZXloeG92bHFqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Mzc5NTUzNzIsImV4cCI6MjA1MzUzMTM3Mn0.JZbYaTngJy3lGFqtvI3efcmxdosdmD48Nv2zgTeaHY0";
...
async function getUserSecurityTier() {
  try {
    ...
    const { data: securityData, error } = await supabase.from("user_security").select("tier, last_updated").eq("id", user.id).single();
    if (error) {
...
async function fetchSystemInfo() {
  ...
  try {
    ...
    if (cpuUsageElement) {
      cpuUsageElement.textContent = typeof info.cpu === "number" ? info.cpu.toFixed(1) : "N/A";
    }
    if (ramUsageElement) {
      ramUsageElement.textContent = typeof info.ram === "number" ? info.ram.toFixed(1) : "N/A";
    }
